...
from Quark.Factor.memory_core import SharedMemoryCore, SyncMemoryCore, NamedVector
from Quark.Factor.utils import FactorMonitor as FactorMonitorBase, ConcurrentMonitorManager as MonitorManager, IndexWeight as IndexWeightBase
...
class IndexWeight(IndexWeightBase):
    def information_coefficient(self, factor_value: dict[str, float], pred_value: dict[str, float]) -> float:
        ...
        # Calculate the covariance matrix of factor values and asset returns
        covariance_matrix = np.cov(selected_factor, selected_pred, bias=True)

        # Extract the covariance between factor values and asset returns
    ...
    def ic(self, factor_value: dict[str, float], pred_value: dict[str, float])-> float:
        return self.information_coefficient(factor_value=factor_value, pred_value=pred_value)
